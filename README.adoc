...
The program found max value between three numbers.

To compile assembler code and run tests, execute this command:

----
 ./quickstart/basic/make.sh test
----

Result should look like this:
----
Build program for file quickstart/basic/genermaxofthree.test.c
==================
Run maxofthree.test
------------------
...
Folder `basic` contains pure c test while `generated_runner` use the text `TEST` to identify test functions and generate a runner that execute all tests. 
...
All sub projects in this folder use the script "script/make.sh" to compile and execute code. 

It's call by the `make.sh` that is within the sub project folder.

This script discover files to build and execute generated command lines.
Those command lines are recorded in `work/rerun.sh` to see what is needed to compile.
This script can be use to replay the building process.
d
.To rerun the script
----
pushd work; ./rerun.sh; popd
----
...
To launch it, execute: `./howto/minimal_test/make.sh make.sh tests`
...
[source, c]
----
include::howto/real_test/simple.test.c[]
----

Script `./howto/real_test/make.sh test` compile and run tests.
...
----
while true; do ./[PATH TO THE PROJECT]/make.sh test; date; sleep 5;  done
----
...
----
while true; do inotifywait -q -r -e create,modify,delete .; ./[PATH TO THE PROJECT]/make.sh test; date; done;
----


=== Check all

.To check that all subproject work:
----
for file in $(find . -name "make.sh"); do  echo $file && $file test > /dev/null || echo -e "\033[0;31mERROR: $file\n============\033[0m"; done
----

.Execute with trace
----
for file in $(find . -name "make.sh"); do  echo $file && $file test || echo -e "\033[0;31mERROR: $file\n============\033[0m"; done
----
...
----
warning: target/maxofthree.asm.o: missing .note.GNU-stack section implies executable stack
NOTE: This behaviour is deprecated and will be removed in a future version of the linker
----

To avoid this error, add this option to the `gcc` command:
`-z noexecstack`
...
gdb [program]: run gdb for th progam +
_(gdb)_ n: **N**ext instruction +
_(gdb)_ s: **S**tep into +
_(gdb)_ i r: **I**nfo **R**egisters display all registers +
_(gdb)_ i r rax: **I**nfo **R**egisters RAX
...
[NOTE]
..
We found a lot of examples that use `0x80` instead of `syscall`. 
It's a legacy way to invoke system call but the only one available for 32-bit. 
In 64-bit, `syscall` is always available.
see: https://stackoverflow.com/questions/15168822/intel-x86-vs-x64-system-call
..


you can find `syscall` codes and parameters in this page: 
